...
end_datetime = datetime(2023, 1, 9, 0, 0, 0)
...
with open(f"result/{parking_name}.json") as f:
    ...
    timezone = pytz.timezone("Asia/Tokyo")
    df["entryTimeDf"] = pd.to_datetime(df["entryTime"]).dt.tz_convert(timezone)
    df["exitTimeDf"] = pd.to_datetime(df["exitTime"]).dt.tz_convert(timezone)
...
while current_datetime < end_datetime:
    ...
    filtered_df = df.loc[
        (df["entryTimeDf"].dt.date == pd.to_datetime(current_datetime).date())
        | (df["exitTimeDf"].dt.date == pd.to_datetime(current_datetime).date())
    ]
    count = 0
    for record in json.loads(filtered_df.to_json(orient="records")):
